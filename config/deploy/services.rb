namespace :services do
  ...
  namespace :install do
    ...
    task :unicorn do
      ...
      upload 'config/deploy/service/unicorn/run', '/var/svc.d/unicorn/run', :mode => 0755
      template = ERB.new(File.read('config/deploy/service/unicorn/unicorn.sh.erb'))
      put template.result(binding), '/var/svc.d/unicorn/unicorn.sh', :mode => 0755
      upload 'config/deploy/service/unicorn/log/run', '/var/svc.d/unicorn/log/run', :mode => 0755
