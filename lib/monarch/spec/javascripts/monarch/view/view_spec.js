  ...
  describe("Views constructed with Monarch.View.Template#toView", function() {
    ...
    describe("#registerInterest(object, methodNome, callback, context)", function() {
      ...
      it("allows a disambiguating name to be provided as a first argument, so that handlers can be registered on two different objects but the same method name", function() {
        ...
        view.registerInterest('a', node1, 'subscribe', callbackA, context);
        view.registerInterest('b', node2, 'subscribe', callbackB, context);
        expect(node1.size()).to(eq, 1);
        expect(node2.size()).to(eq, 1);

        node1.publish("foo");
        expect(callbackA).to(haveBeenCalled, withArgs("foo"));
        expect(callbackA.mostRecentThisValue).to(eq, context);
        ...
        expect(callbackB.mostRecentThisValue).to(eq, context);
        expect(callbackA).toNot(haveBeenCalled);

        callbackB.clear();

        view.registerInterest('a', node3, 'subscribe', callbackA, context);
        expect(node1.size()).to(eq, 0);
        expect(node2.size()).to(eq, 1);
