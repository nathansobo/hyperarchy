...
$.extend(Disco.prototype, {
  tag: function() {
    ...
    if (this.is_self_closing(tag_name)) {
      this.doc.push(new Disco.SelfClosingTag(tag_name, attributes));
    } else {
      this.doc.push(new Disco.OpenTag(tag_name, attributes));
  ...
  self_closing_tag_hash: { 'br': 1, 'hr': 1, 'input': 1, 'img': 1 },
  
  is_self_closing: function(tag_name){
...
Disco.SelfClosingTag = function(tag_name, attributes) {
  this.tag_name = tag_name;
  this.attributes = attributes;
}

$.extend(Disco.SelfClosingTag.prototype, {
  to_string: function() {
    var serialized_attributes = [];
    for(var attributeName in this.attributes) {
      serialized_attributes.push(attributeName + '="' + this.attributes[attributeName] + '"');
