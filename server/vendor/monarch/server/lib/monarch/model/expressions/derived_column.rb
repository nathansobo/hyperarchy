module Model
  module Expressions
    class DerivedColumn < Column
      ...
      delegate :convert_value_for_storage, :convert_value_for_wire, :type, :aggregation?, :to => :expression
      ...
      def sql_derived_column(state)
        state[self][:sql_derived_column] ||= begin
          ...
          Sql::DerivedColumn.new(relation.internal_sql_table_ref(state), expression.sql_expression(state), sql_name)
        end
      end
